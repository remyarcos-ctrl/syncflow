...
export default function StockPage() {
  ...
  const genererBon = async () => {
    ...
    try {
      ...
      const lignes = items.map((it) => `<tr><td><a href="${origin}/exceptions?ref=${encodeURIComponent(it.ref)}" target="_blank" style="color:#4f46e5;font-weight:600;text-decoration:none">${esc(it.ref)} ↗</a></td><td>${esc(it.desig)}</td><td style="font-weight:700">${esc(it.emplacement || '—')}</td><td style="text-align:right">${it.stock ?? '—'}</td><td style="text-align:right">${it.ecart != null ? (Number(it.ecart) > 0 ? '+' : '') + Number(it.ecart) : ''}</td><td>${esc(it.bon)}</td><td></td><td></td></tr>`).join('');
